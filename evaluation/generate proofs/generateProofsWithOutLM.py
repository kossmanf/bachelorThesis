...
_parameters = ['--auto', '--silent', '--training-examples=3', '--proof-object', '--cpu-limit=20', '-R']
...
if not os.path.exists(_proofLogFilePath):
    ...
    if os.path.exists('./proofsNormal'):
        # Delete existing proofs
        for category in os.listdir('./proofsNormal'):
            category_path = os.path.join('./proofsNormal/', category)
            for proof in os.listdir(category_path):
                os.remove(os.path.join(category_path, proof))
            os.rmdir(category_path)
    else:
        os.mkdir('./proofsNormal')
...
with open(_proofLogFilePath, "r+") as proof_log:
    ...
    if os.path.exists(_axiomsPath):
        ...
        for category in os.listdir(_axiomsPath):
            ...
            category_proof_path = os.path.join('.', 'proofsNormal', category)
